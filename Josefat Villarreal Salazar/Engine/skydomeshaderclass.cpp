...
bool SkydomeShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
								...
								ID3D11ShaderResourceView** texture, int textureCount)
{
	bool result;


	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, ambientColor, diffuseColor, lightDirection, texture, textureCount);
	if(!result)
	{
		return false;
	}
...
bool SkydomeShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	lightBufferDesc.ByteWidth = sizeof(BufferType);
											 ...
											 ID3D11ShaderResourceView** texture, int textureCount)
{
	...
	// Get a pointer to the data in the constant buffer.
	dataPtr2 = (BufferType*)mappedResource.pData;
	...
	deviceContext->PSSetShaderResources(0, textureCount, texture);
